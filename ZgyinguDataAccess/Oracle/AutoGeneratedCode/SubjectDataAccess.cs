...
namespace Zkjj.Zgyinggu.DataAccess.SQLServer
{
	...
	public class SubjectDataAccessBase
	{
		...
		{
			string commandName = "Pr_Subject_I";
		...
		{
			string commandName = "Pr_Subject_D";
		...
		{
			string commandName = "Pr_Subject_U";
		...
		{
			string commandText = @"SELECT S_Subject.nextval FROM DUAL;";
		...
		public virtual Subject GetById(Int32 subjectId)
		{
			...
			string commandName = "Query_Subject_GetByPk";
			string commandText = @"
SELECT *
FROM Subject
WHERE
 SubjectId = :p_SubjectId";
		...
		public virtual SubjectList GetPagedListAll(int startRowIndex, int maximumRows, string sortExpression, out int totalRecords)
		{
			...
			string commandName = "Query_Subject_GetPagedListAll";
			string commandText = string.Format(@"
SELECT * FROM 
(SELECT a.*, ROWNUM RIndex FROM 
(SELECT * FROM Subject t {0}) a
...
SELECT COUNT(*) FROM Subject";
		...
		/// <param name="totalRecords">（返回）记录总数</param>
		/// <returns>Subject列表</returns>
		public virtual SubjectList GetPagedListByCriteria(int startRowIndex, int maximumRows, string sortExpression, string criteria, out int totalRecords)
		{
			SubjectList subjectList = new SubjectList();
			
			string commandName = "Query_Subject_GetPagedListByCriteria";
			string commandText = string.Format(@"
SELECT * FROM 
(SELECT a.*, ROWNUM RIndex FROM 
(SELECT * FROM Subject t WHERE 1=1 {1} {0}) a
) b WHERE RIndex > :p_startRowIndex AND RIndex <= :p_endRowIndex",
				string.IsNullOrEmpty(sortExpression) ? "" : "ORDER BY " + sortExpression,
				criteria);
...
SELECT COUNT(*) FROM Subject WHERE 1=1 {0} ",
				criteria);;
			totalRecords = Convert.ToInt32(SqlHelper.ExecuteScalar(ConnectionString.ZgyinguRead, CommandType.Text, commandText, null));
